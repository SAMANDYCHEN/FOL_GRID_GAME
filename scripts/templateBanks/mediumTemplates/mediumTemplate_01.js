...
function replacePlaceholders(template, details) {
    return template
        ...
        .replace(/{condition}/g, details.condition === 'greater' ? 'greater than' : 'less than')
...
export const mediumTemplate_01 = {
    generateStatements() {
        ...
        // "All circles with value greater than 6 are Red and located in the left 2 columns of the grid."
        ...
        // FOL statement:
        // "∀x ((Shape(x, shape1) ∧ Value(x) > threshold) → (Color(x, colorName) ∧ Location(x, region)))"
        // The "Location" part is conceptual/pedagogical, as your puzzle uses grid-based checks.
        const formalFOLStatement = `
        ∀x (
          (Shape(x, ${shape1}) ∧ Value(x) ${condition === 'greater' ? '>' : '<'} ${numberThreshold})
    ...
    generateGrid(satisfies, details) {
        ...
        const { direction, numUnits, color1, shape1, condition, numberThreshold } = details;
        ...
        if (satisfies) {
            ...
            // Also ensure that outside the region or with different shapes/values we don't accidentally form partial matches.

            grid.forEach(cell => {
                // If a cell meets shape1 + value condition, enforce color + region
                if (cell.shape === shape1 && valueMatches(cell)) {
                    cell.color = color1;
                    // If it's not in the region, forcibly move it into region or fix attributes
                    if (!inRegion(cell)) {
                        // best approach: either move it or break the condition
                        // We'll break the condition so it no longer meets "shape1 + value condition"
                        // i.e. change shape or number
                        cell.shape = getRandomElement(shapes.filter(s => s !== shape1));
                    }
                } else {
                    // If this cell is in the region, we want to ensure it doesn't accidentally fulfill shape+value
                    if (inRegion(cell)) {
                        ...
                        // outside region → cannot match shape1+value with color1
                        if (cell.shape === shape1 && valueMatches(cell) && cell.color === color1) {
            ...
            // We want at least one violation: find a cell that "should" meet shape+value => color+region, but break it.

            // Step 1: partially try to satisfy the statement to create potential correct cells
            grid.forEach(cell => {
                if (cell.shape === shape1 && valueMatches(cell) && inRegion(cell)) {
                    cell.color = color1;
                }
            });

            // Step 2: Introduce a violation
            // For at least one cell that "ought to" be color1 & inRegion, we do something to break it.
            const target = grid.find(cell =>
                cell.shape === shape1 && valueMatches(cell) && inRegion(cell) && (cell.color === color1)
            );
            if (target) {
                // break it: e.g. change color to something else
                target.color = getRandomElement(colors.filter(c => c !== color1));
            }
        }
    ...
    verifyStatementWithGrid(grid, details) {
        const { direction, numUnits, shape1, color1, condition, numberThreshold } = details;
        ...
        // they must be color1 & in the region. Also, outside region shouldn't incorrectly have color1 if shape1+valueMatches”
        return grid.every(cell => {
            ...
            // If a cell is shape1 + valueMatches + color1 but NOT inRegion => violation
