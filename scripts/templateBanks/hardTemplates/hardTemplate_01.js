...
function replacePlaceholders(template, details) {
    // Example placeholders: {shape1}, {shape2}, {color1}, {valueConditionDescription}
    return template
...
export const hardTemplate_01 = {
    generateStatements() {
        ...
        const valueConditionDescription = `with a value greater than ${valueThreshold}`;
        ...
        // Natural language
        // "For all shape1s that are color1, there exists a shape2 with value > threshold directly to the right."
        const naturalLanguageStatement = replacePlaceholders(
            "For all {shape1}s that are {color1}, there exists a {shape2} {valueConditionDescription} directly to the right of it.",
            { ...details, color1: colorName }
        );

        // FOL: ∀x((shape(x)=shape1 ∧ color(x)=color1) → ∃y(shape(y)=shape2 ∧ value(y)>threshold ∧ RightOf(y,x)))
        const formalFOLStatement = `
        ∀x (
          (Shape(x, ${shape1}) ∧ Color(x, ${colorName}))
          → ∃y (Shape(y, ${shape2}) ∧ Value(y) > ${valueThreshold} ∧ RightOf(y, x))
        )
      `.trim();
    ...
    generateGrid(satisfies, details) {
        ...
        if (satisfies) {
            // We want: for every X that is shape1+color1, ∃ Y to its right that is shape2 and number>valueThreshold.
            // If X is in col=4 (last column), we cannot find a right neighbor, so we must fix X so it doesn’t match shape1+color1.
            ...
            grid.forEach(cellX => {
                if (cellX.shape === shape1 && cellX.color === color1) {
                    ...
                    if (cellX.position.col === gridSize - 1) {
                        ...
                        let theRight = cellToRight(cellX);
                        // If theRight doesn’t meet shape2 + number>threshold, fix it
                        if (!theRight) return; // theoretically won't happen except col=4
                        if (theRight.shape !== shape2 || theRight.number <= valueThreshold) {
                            theRight.shape = shape2;
                            theRight.number = randomIntFromInterval(valueThreshold + 1, 10);
                            // color can be anything as it's not relevant to the statement
                        }
                    }
                }
            });
        } else {
            // We want at least one violation. That means at least one X (shape1+color1) that does NOT have a suitable Y to the right
            // Step 1: partially satisfy
            grid.forEach(cellX => {
                if (cellX.shape === shape1 && cellX.color === color1 && cellX.position.col < gridSize - 1) {
                    const rightCell = cellToRight(cellX);
                    if (rightCell) {
                        // make that cell shape2 + number>threshold to "correctly" satisfy
                        rightCell.shape = shape2;
                        if (rightCell.number <= valueThreshold) {
                            rightCell.number = randomIntFromInterval(valueThreshold + 1, 10);
                        }
                    }
                }
            });
            ...
            const violatingX = grid.find(cellX => {
                if (cellX.shape === shape1 && cellX.color === color1 && cellX.position.col < gridSize - 1) {
                    const rightCell = cellToRight(cellX);
                    if (rightCell && rightCell.shape === shape2 && rightCell.number > valueThreshold) {
                        return true;
                    }
                    return false;
                }
            ...
            if (violatingX) {
                // break the adjacency: e.g. modify the right cell's shape so it no longer meets shape2
                const rightCell = cellToRight(violatingX);
                if (rightCell) {
                    rightCell.shape = getRandomElement(shapes.filter(s => s !== shape2));
                }
            } else {
                // If we didn't find such an X, we can also break an X in the last column by making sure it's shape1+color1
                // This ensures there's no right neighbor. That alone is a violation because "∃y to the right" can't be satisfied.
    ...
    verifyStatementWithGrid(grid, details) {
        ...
        // For every X that is shape1+color1, there must exist a Y to the right with shape2 & number>threshold
        const cellToRight = (cellX) => {
            const { row, col } = cellX.position;
        ...
        for (const cellX of grid) {
            if (cellX.shape === shape1 && cellX.color === color1) {
                ...
                // We need at least 1 neighbor Y fulfilling shape2 + number>threshold
