...
export const hardTemplate_04 = {
    generateStatements() {
        ...
        // "For all shape1 that are color1, there exists shape2 with value< threshold directly below it."
        const naturalLanguageStatement = replacePlaceholders(
            "For all {shape1}s that are {color1}, there exists a {shape2} {valueConditionDescription} directly below it.",
        ...
        ∀x (
          ...
          → ∃y (Shape(y, ${shape2}) ∧ Value(y) < ${valueThreshold} ∧ Below(y, x))
        )
      `.trim();
    ...
    generateGrid(satisfies, details) {
        ...
        const cellBelow = (cellX) => {
            ...
            return grid.find(c => c.position.row === row + 1 && c.position.col === col);
        };
        ...
        if (satisfies) {
                ...
                if (cellX.shape === shape1 && cellX.color === color1) {
                    ...
                    if (cellX.position.row === gridSize - 1) {
                        ...
                        if (below) {
                            ...
                            if (below.shape !== shape2 || below.number >= valueThreshold) {
                                below.shape = shape2;
                                below.number = randomIntFromInterval(1, valueThreshold - 1);
                            }
                        }
                    }
                }
            ...
            // partially satisfy
            grid.forEach(cellX => {
                if (cellX.shape === shape1 && cellX.color === color1 && cellX.position.row < gridSize - 1) {
                    const bCell = cellBelow(cellX);
                    if (bCell) {
                        bCell.shape = shape2;
                        if (bCell.number >= valueThreshold) {
                            bCell.number = randomIntFromInterval(1, valueThreshold - 1);
                        }
                    }
                }
            });
            ...
            const violatingX = grid.find(cellX => {
                if (cellX.shape === shape1 && cellX.color === color1 && cellX.position.row < gridSize - 1) {
                    const bCell = cellBelow(cellX);
                    if (bCell && bCell.shape === shape2 && bCell.number < valueThreshold) {
                        return true;
                    }
                }
                return false;
            ...
            if (violatingX) {
                ...
                const bottomCell = grid.find(c => c.position.row === gridSize - 1);
                if (bottomCell) {
